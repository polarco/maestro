...
import { SessionTimeline } from "@renderer/components/conversation/timeline";
import { StudioPanel } from "@renderer/components/studio-panel";
...
const strategies: Array<{ id: TurnPath; label: string; description: string }> = [
  { id: "answer", label: "Responder", description: "Resposta direta, sem mutações" },
  { id: "research", label: "Pesquisar", description: "Investigar fontes e workspace" },
  { id: "plan", label: "Planejar", description: "Construir um plano revisável" },
  { id: "execute", label: "Executar", description: "Somente após limites aprovados" },
];

function inferStrategyPreview(content: string): TurnPath {
  const value = content.trim();
  if (
    /^\/(?:review|research)\b/i.test(value) ||
    /\b(?:pesquise|investigue|analise|revise|encontre)\b/i.test(value)
  )
    return "research";
  if (/\b(?:aprovo|pode executar|execute o plano|go ahead)\b/i.test(value)) return "execute";
  if (
    /^\/plan\b/i.test(value) ||
    /\b(?:implemente|crie|adicione|altere|corrija|refatore|remova)\b/i.test(value)
  )
    return "plan";
  return "answer";
}
...
export function ConversationPage({
  ...
  const [mode, setMode] = useState<RunMode>("maestro");
  ...
  const [strategyOverride, setStrategyOverride] = useState<TurnPath | null>(null);
  const [editingTurnId, setEditingTurnId] = useState<string | null>(null);
  const [studioOpen, setStudioOpen] = useState(() => {
    const saved = window.localStorage.getItem("maestro.studio-open");
  ...
  const branchComparison = useQuery({
    queryKey: ["branch-comparison", id, ...(detail?.branches?.map((branch) => branch.id) ?? [])],
    queryFn: async () => {
      const branches = detail?.branches ?? [];
      return Promise.all(
        branches.slice(-4).map(async (branch) => {
          const page = await api().getSessionTimeline(id, undefined, 500, branch.id);
          ...
          return {
            branch,
            content: response?.kind === "message" ? response.message.content : "Sem resposta.",
            createdAt: response?.createdAt ?? branch.createdAt,
          };
        }),
      );
    },
    enabled: branchComparisonOpen && (detail?.branches?.length ?? 0) > 1,
  });

  useEffect(() => {
    if (!branchComparisonOpen) return;
    const close = (event: KeyboardEvent) => {
      if (event.key === "Escape") setBranchComparisonOpen(false);
    };
  ...
  useEffect(() => {
    window.localStorage.setItem("maestro.studio-open", String(studioOpen));
  }, [studioOpen]);
  ...
  const send = useMutation({
    mutationFn: async () => {
      ...
      return editingTurnId
        ? api().editTurn({ ...turnInput, turnId: editingTurnId })
        : api().sendTurn(turnInput);
    },
    onSuccess: (value) => {
      if (!value) return;
  ...
  const optimizationLabel =
    bootstrap.settings.tokenOptimizationMode === "balanced"
      ...
      : bootstrap.settings.tokenOptimizationMode === "aggressive"
        ? "agressiva"
        : "desativada";
  const inferredStrategy = inferStrategyPreview(content);
  const activeStrategy = strategyOverride ?? inferredStrategy;
  const strategy = strategies.find((item) => item.id === activeStrategy)!;

  return (
    <div className="page-enter relative flex h-full min-w-0 bg-bg/45">
      <section className="flex min-w-0 flex-1 flex-col">
        <header className="flex min-h-[66px] shrink-0 items-center gap-3 border-b border-border bg-bg/75 px-4 py-2.5 md:px-5">
          ...
          {(detail.branches?.length ?? 0) > 1 ? (
            <div className="flex items-center gap-1.5">
              <label className="hidden items-center gap-2 rounded-[9px] border border-border bg-surface px-2.5 py-1.5 text-[10px] text-text-muted lg:flex">
                <GitBranch size={11} />
                <select
                  className="max-w-36 bg-transparent text-[10px] outline-none"
                  value={detail.activeBranchId ?? ""}
                  onChange={(event) => {
                    void api()
                      .switchBranch(id, event.target.value)
                      .then(() => {
                        void queryClient.invalidateQueries({ queryKey: ["conversation", id] });
                        void queryClient.invalidateQueries({ queryKey: ["timeline", id] });
                      });
                  }}
                  aria-label="Branch da conversa"
                >
              ...
              <button
                ...
                onClick={() => setBranchComparisonOpen(true)}
                title="Comparar respostas entre branches"
                aria-label="Comparar respostas entre branches"
              >
                <Columns2 size={12} /> <span className="hidden xl:inline">Comparar</span>
              </button>
            </div>
          ) : null}
          <button
            className={cn("studio-header-button", studioOpen && "is-active")}
            onClick={() => setStudioOpen((value) => !value)}
            aria-pressed={studioOpen}
          >
            <PanelRightOpen size={13} /> <span className="hidden sm:inline">Studio</span>
          </button>
          ...
          <div className="mx-auto flex min-h-full max-w-[940px] flex-col px-5 py-7 md:px-7">
            {detail.messages.length === 0 ? (
              ...
              <SessionTimeline
                sessionId={id}
                project={project}
                streaming={detail.messages.some((message) => message.status === "streaming")}
                onEdit={(turnId, value) => {
                ...
                onContinue={(value) => {
                  setContent(`Continue a partir desta resposta:\n\n${value}`);
                  window.requestAnimationFrame(() => composerRef.current?.focus());
                }}
        ...
        <footer className="shrink-0 border-t border-border bg-bg/90 px-4 py-3 backdrop-blur-xl md:px-5">
          <div className="mx-auto max-w-[940px]">
            <div className="relative mb-2 flex flex-wrap items-center gap-2">
              <div className="relative">
                <button
                  className={cn(
                    "strategy-chip",
                    `is-${activeStrategy}`,
                    strategyOverride && "is-overridden",
                  )}
                  onClick={() => {
                    setStrategyMenu((value) => !value);
                    setModeMenu(false);
                    setConfigurationOpen(false);
                    setFastModelOpen(false);
                  }}
                  aria-expanded={strategyMenu}
                  title={
                    strategyOverride
                      ? "Estratégia definida por você"
                      : "Estratégia inferida pelo Maestro"
                  }
                >
                  <Sparkles size={12} />
                  <span>{strategy.label}</span>
                  <span className="strategy-chip-source">
                    {strategyOverride ? "manual" : "auto"}
                  </span>
                  <ChevronDown size={10} />
                </button>
                {strategyMenu ? (
                  <div className="glass-popover absolute bottom-11 left-0 z-50 w-72 rounded-[13px] p-1.5">
                    <div className="flex items-center justify-between px-2.5 pb-1.5 pt-1 text-[9px] font-semibold uppercase tracking-[0.12em] text-text-faint">
                      Estratégia do turno
                      {strategyOverride ? (
                        <button
                          className="normal-case tracking-normal text-primary-soft"
                          onClick={() => {
                            setStrategyOverride(null);
                    ...
                    {strategies.map((item) => (
                      <button
                        key={item.id}
                        className={cn("strategy-option", activeStrategy === item.id && "is-active")}
                        onClick={() => {
                          setStrategyOverride(item.id);
                          setStrategyMenu(false);
                        }}
                      >
                        <span className={cn("strategy-dot", `is-${item.id}`)} />
                        <span className="min-w-0 flex-1">
                          <strong>{item.label}</strong>
                          <small>{item.description}</small>
                        </span>
                        {activeStrategy === item.id ? (
                          <CheckCircle2 size={12} className="text-primary-soft" />
                        ) : null}
                      </button>
                    ))}
                  </div>
              ...
              <div className="ml-auto flex h-9 items-center gap-2 rounded-[9px] border border-border/75 bg-bg-elevated/70 px-3 text-[10px] text-text-faint">
                ...
                <span className="hidden sm:inline">
                  {autonomyQuery.data?.level === "observe"
                    ? "Observe · somente leitura"
                    : autonomyQuery.data?.level === "autopilot"
                      ? "Autopilot · escopo pré-aprovado"
                      : "Review · escrita após aprovação"}
                </span>
            ...
            >
                ...
                placeholder={
                  sessionKind === "pty"
                    ...
                    : awaitingClarification
                      ...
                      : editingTurnId
                        ? "Edite o pedido; uma nova branch será criada…"
              ...
              <div className="flex items-center gap-1 px-1 pt-1">
                ...
                <span className="ml-1 hidden text-[10px] text-text-faint sm:inline">
                  /plan · /review · /compact · @contexto · Enter envia
                </span>
                <Button
                  className="ml-auto"
                  size="md"
            ...
            {visionBlocked ? (
              ...
              >
                <AlertTriangle size={11} /> A imagem exige um modelo com visão. Escolha outro
                modelo.
              </button>
            ) : null}
            {videoFramesOmitted ? (
      ...
      {studioOpen ? (
        <StudioPanel
          project={project}
          sessionId={id}
          branchId={detail.activeBranchId ?? null}
      ...
      {branchComparisonOpen ? (
        <div
          ...
          onMouseDown={(event) => {
            if (event.target === event.currentTarget) setBranchComparisonOpen(false);
          }}
        >
          <section
            className="flex max-h-[82vh] w-full max-w-5xl flex-col overflow-hidden rounded-[16px] border border-border bg-sidebar shadow-2xl"
            role="dialog"
            aria-modal="true"
            aria-label="Comparar respostas entre branches"
          >
            <header className="flex items-center gap-3 border-b border-border px-5 py-4">
              ...
              <div className="min-w-0 flex-1">
                <h2 className="text-[13px] font-semibold text-text">Comparar respostas</h2>
                <p className="mt-0.5 text-[9px] text-text-faint">
                  Até quatro versões, sem duplicar o histórico compartilhado
                </p>
              </div>
              <button
            ...
            <div className="grid min-h-0 flex-1 gap-3 overflow-auto p-4 md:grid-cols-2">
              {branchComparison.isLoading ? (
                <div className="col-span-full grid min-h-60 place-items-center text-text-faint">
                  <LoaderCircle size={20} className="animate-spin" />
                </div>
              ) : null}
              {branchComparison.data?.map(({ branch, content: response, createdAt }) => (
                <article
                  key={branch.id}
                  className="flex min-h-64 min-w-0 flex-col rounded-[12px] border border-border bg-bg-elevated"
                >
                  <header className="flex items-center gap-2 border-b border-border px-3 py-2.5">
                    <GitBranch size={11} className="text-primary-soft" />
                    <strong className="min-w-0 flex-1 truncate text-[10.5px] text-text">
                      {branch.name}
                    </strong>
                    <time className="text-[8px] text-text-faint">
                    ...
                    <button
                      ...
                      aria-label={`Copiar resposta de ${branch.name}`}
                    >
                      <Clipboard size={10} />
                    </button>
                  </header>
                  <div className="min-h-0 flex-1 overflow-auto whitespace-pre-wrap p-3 text-[11px] leading-5 text-text-muted">
                    {response}
                  </div>
                </article>
              ))}
              {branchComparison.error ? (
                <p className="col-span-full rounded-[9px] border border-danger/20 p-3 text-[10px] text-danger">
                  {branchComparison.error instanceof Error
                    ? branchComparison.error.message
                    : String(branchComparison.error)}
                </p>
              ) : null}
            </div>
          </section>
